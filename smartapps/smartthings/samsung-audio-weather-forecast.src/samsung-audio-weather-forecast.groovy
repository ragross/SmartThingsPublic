/**
 *  Samsung Audio Weather Forecast
 *
 *  Author: SmartThings
 *  Date: 2014-11-11
 */
...
definition(
    name: "Samsung Audio Weather Forecast",
    namespace: "smartthings",
    author: "olivia ju",
    description: "Play a weather report through your Samsung Audio when the mode changes or other events occur",
    category: "My Apps",
...
preferences {
	page(name: "mainPage", title: "Play the weather report on your Samsung Audio", install: true, uninstall: true)
...
def mainPage() {
	dynamicPage(name: "mainPage") {
		...
		section {
			input "SamsungAudio", "capability.musicPlayer", title: "On this Samsung Audio player", required: true
		}
...
private takeAction(evt) {
	...
	if (song) {
		SamsungAudio.playSoundAndTrack(state.sound.uri, state.sound.duration, state.selectedSong, volume)
	}
	else if (resumePlaying){
		SamsungAudio.playTrackAndResume(state.sound.uri, state.sound.duration, volume)
	}
	else if (volume) {
		SamsungAudio.playTrackAtVolume(state.sound.uri, volume)
	}
	else {
		SamsungAudio.playTrack(state.sound.uri)
	}

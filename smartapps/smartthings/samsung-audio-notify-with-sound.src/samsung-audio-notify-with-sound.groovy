 ...
 */
definition(
    name: "Samsung Audio Notify with Sound",
    namespace: "smartthings",
    author: "olivia ju",
	description: "Play a sound or custom message through your Samsung when the mode changes or other events occur.",
    category: "My Apps",
...
preferences {
	page(name: "mainPage", title: "Play a message on your Samsung when something happens", install: true, uninstall: true)
...
private songOptions() {
	...
	def states = SamsungAudio.statesSince("trackData", new Date(0), [max:30])
...
private saveSelectedSong() {
	try {
		...
		def songs = SamsungAudio.statesSince("trackData", new Date(0), [max:30]).collect{it.jsonValue}

 ...
 */
definition(
    name: "Roomie Agent Trigger",
    namespace: "roomieremote-ratrigger",
    author: "Roomie Remote, Inc.",
    description: "Trigger Roomie activities when certain actions take place in your home.",
    category: "My Apps",
...
preferences {
	page(name: "agentSelection", title: "Select your Roomie Agent")
	page(name: "refreshActivities", title: "Updating list of Roomie activities")
    page(name: "control", title: "Run a Roomie activity when something happens")
...
def agentSelection()
{
	...
	dynamicPage(name: "agentSelection", title: "Select your Roomie Agent", nextPage: "control", install: false, uninstall: true) {
		section {
			input "agent", "capability.mediaController", title: "Roomie Agent", required: true, multiple: false
		}
	}
}
...
def control()
{
    ...
    if (!activities || !state.refreshCount)
    {
        ...
        dynamicPage(name: "control", title: "Updating list of Roomie activities", nextPage: "", refreshInterval: refreshInterval, install: false, uninstall: true) {
        	section("") {
            	paragraph "Retrieving activities from Roomie Agent"
            }
        }
	}
	else
    {
	dynamicPage(name: "control", title: "Run a Roomie activity when something happens", nextPage: "timeIntervalInput", install: false, uninstall: true) {

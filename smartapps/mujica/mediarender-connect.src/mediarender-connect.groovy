/**
 *  MediaRenderer Service Manager v 1.5.1
 *
...
def addMediaRenderer() {
	...
	selectedMediaRenderer.each { dni ->
		...
		if(!d) {
			...
			if (newPlayer){
				//MediaRenderer
				d = addChildDevice("mujica", "MediaRenderer", dni, newPlayer?.value.hub, [label:"${newPlayer?.value.name} Speaker","data":["model":newPlayer?.value.model,"avtcurl":newPlayer?.value.avtcurl,"avteurl":newPlayer?.value.avteurl,"rccurl":newPlayer?.value.rccurl,"rceurl":newPlayer?.value.rceurl,"udn":newPlayer?.value.udn,"dni":dni]])
			}
			runSubscribe = true
		} 
	}

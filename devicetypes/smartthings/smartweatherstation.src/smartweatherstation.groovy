...
metadata {
  ...
  definition (name: "SmartWeatherStation*", namespace: "smartthings", author: "SmartThings") {
  	capability "Polling"
  ...
  tiles {
    ...
    standardTile("water", "device.water", decoration: "flat") {
      ...
      state "true",    label:'Wet',          icon: "st.alarm.watya yer.wet",        backgroundColor:"#ff9999"
    ...
    main(["temperature", "weatherIcon","feelsLike", "water"])
    details(["temperature", "humidity", "weatherIcon","water", "feelsLike","wind","weather", "city","percentPrecip", "refresh","alert","rise","set","light"])}
}
...
def poll() {
  ...
  if (obs) {
    ...
    if (waterConditions(obs.weather) == true) {
      send( name: "water", value: "true", isStateChange: true )
    } else {
      send( name: "water", value: "false", isStateChange: true )
    }
...
private waterConditions(weather)
{
  def weatherCurrent = weather
  log.debug "Current: ${weatherCurrent}"
  def waterFlag = false
  if(weatherCurrent.contains("flurries")) {
      waterFlag = true
  } else if(weatherCurrent.contains("sleet")) {
      waterFlag = true
  } else if(weatherCurrent.contains("rain")) {
      waterFlag = true
  } else if(weatherCurrent.contains("snow")) {
      waterFlag = true
  } else if(weatherCurrent.contains("storms")) {
      waterFlag = true
  }
  waterFlag
}
